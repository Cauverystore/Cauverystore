...
const CreditNoteView = () => {
  ...
  const handleDownloadPdf = async () => {
    setPdfLoading(true);
    try {
      const res = await api.get(`/api/gst/creditnote/${id}/pdf`, { responseType: "blob" });
      ...
      if (err.response?.status === 403) setError("Access denied: You do not have permission to download this credit note.");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get(`/api/gst/creditnote/${id}`);
        setCreditNote(res.data.creditNote);
      } catch (err) {
        if (err.response?.status === 403) setError("Access denied: You do not have permission to view this credit note.");
        else setError("Failed to load credit note");
      }
      setLoading(false);
    };
    fetchData();
  ...
  if (loading) return <div className="giv-page" style={{ textAlign: "center", padding: "3rem", color: "#94a3b8" }}>Loading credit note...</div>;
  if (error) return <div className="giv-page" style={{ textAlign: "center", padding: "3rem", color: "#dc2626" }}>{error}</div>;
  if (!creditNote) return <div className="giv-page" style={{ textAlign: "center", padding: "3rem", color: "#94a3b8" }}>Credit note not found.</div>;

  const cn = creditNote;
  const hasCess = (cn.items || []).some((i) => (i.cessAmount || 0) > 0);

  return (
    <>
      <style>{GST_CREDIT_NOTE_STYLES}</style>
      <div className="giv-page">
        ...
        <div className="giv-invoice" id="credit-note-print">
            ...
            <div className="giv-header-left">
              ...
              <div>
                ...
                <div style={{ fontSize: "0.78rem", color: "#64748b" }}>
                  Original for Recipient{cn.isInterState ? " (Inter-State)" : " (Intra-State)"}
                </div>
              </div>
            </div>
            <div className="giv-header-right">
              <div className="giv-inv-label">Credit Note No.</div>
              <div className="giv-inv-number">{cn.creditNoteNumber}</div>
              <div className="giv-inv-label" style={{ marginTop: "0.3rem" }}>Date</div>
              <div style={{ fontSize: "0.88rem", fontWeight: 600 }}>{cn.creditNoteDate}</div>
              {cn.originalInvoiceNumber && (
                <>
                  <div className="giv-inv-label" style={{ marginTop: "0.3rem" }}>Against Invoice</div>
                  <div style={{ fontSize: "0.82rem", fontFamily: "monospace", fontWeight: 600 }}>{cn.originalInvoiceNumber}</div>
                </>
              )}
            </div>
          </div>
          ...
          <div className="giv-body">
            <div className="giv-parties">
              <div className="giv-party-box">
                ...
                <div className="giv-party-name">{cn.sellerLegalName || "Cauvery Store"}</div>
                <div className="giv-party-detail">{cn.sellerAddress || ""}</div>
                <div className="giv-party-gstin">GSTIN: {cn.sellerGstin}</div>
              </div>
              <div className="giv-party-box">
                <h3>Buyer (Recipient)</h3>
                <div className="giv-party-name">{cn.buyerName}</div>
                <div className="giv-party-detail">{cn.buyerAddress}</div>
                <div className="giv-party-gstin">{cn.buyerGstin === "URP" ? "GSTIN: URP (Unregistered Person)" : "GSTIN: " + cn.buyerGstin}</div>
              </div>
            </div>
            ...
            <div className="giv-identifiers">
              <div className="giv-id-item"><div className="giv-id-label">Place of Supply</div><div className="giv-id-value">{cn.placeOfSupply || "-"}</div></div>
              <div className="giv-id-item"><div className="giv-id-label">Supply Type</div><div className="giv-id-value">{cn.isInterState ? "Inter-State (IGST)" : cn.utgstApplied ? "Intra-State (CGST+UTGST)" : "Intra-State (CGST+SGST)"}</div></div>
              <div className="giv-id-item"><div className="giv-id-label">Invoice Type</div><div className="giv-id-value">{cn.invoiceType || "B2C"}</div></div>
              <div className="giv-id-item"><div className="giv-id-label">Order</div><div className="giv-id-value">#{cn.orderId}</div></div>
              <div className="giv-id-item"><div className="giv-id-label">Reference</div><div className="giv-id-value">{cn.referenceType || "-"}</div></div>
              <div className="giv-id-item"><div className="giv-id-label">Status</div><div className="giv-id-value">{cn.status}</div></div>
            </div>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    {!cn.isInterState && <><th style={{ textAlign: "right" }}>CGST</th><th style={{ textAlign: "right" }}>{cn.utgstApplied ? "UTGST" : "SGST"}</th></>}
                    {cn.isInterState && <th style={{ textAlign: "right" }}>IGST</th>}
                ...
                <tbody>
                  {(cn.items || []).map((item, i) => (
                    <tr key={i}>
                      ...
                      {!cn.isInterState && <><td className="giv-amt">{(item.cgstRate || 0)}%<br />&#8377;{(item.cgstAmount || 0).toFixed(2)}</td><td className="giv-amt">{(item.sgstRate || 0)}%<br />&#8377;{(item.sgstAmount || 0).toFixed(2)}</td></>}
                      {cn.isInterState && <td className="giv-amt">{(item.igstRate || 0)}%<br />&#8377;{(item.igstAmount || 0).toFixed(2)}</td>}
            ...
            <div className="giv-summary">
              <table className="giv-summary-table">
                <tbody>
                  <tr><td>Taxable Amount</td><td>&#8377;{(cn.taxableAmount || 0).toFixed(2)}</td></tr>
                  {!cn.isInterState && <><tr><td>CGST</td><td style={{ color: "#2563eb" }}>&#8377;{(cn.cgstAmount || 0).toFixed(2)}</td></tr><tr><td>{cn.utgstApplied ? "UTGST" : "SGST"}</td><td style={{ color: "#7c3aed" }}>&#8377;{(cn.sgstAmount || 0).toFixed(2)}</td></tr></>}
                  {cn.isInterState && <tr><td>IGST</td><td style={{ color: "#d97706" }}>&#8377;{(cn.igstAmount || 0).toFixed(2)}</td></tr>}
                  <tr><td>Total Tax</td><td>&#8377;{(cn.totalTax || 0).toFixed(2)}</td></tr>
                  {cn.totalCess > 0 && <tr><td>Compensation Cess</td><td style={{ color: "#b45309" }}>&#8377;{(cn.totalCess || 0).toFixed(2)}</td></tr>}
                  {cn.tcsAmount > 0 && <tr><td>TCS</td><td style={{ color: "#dc2626" }}>&#8377;{(cn.tcsAmount || 0).toFixed(2)}</td></tr>}
                  <tr className="giv-sum-total"><td>Total Amount Credited</td><td>&#8377;{(cn.totalAmount || 0).toFixed(2)}</td></tr>
                </tbody>
              </table>
            </div>

            <div className="giv-amount-words">
              <strong>Amount in Words:</strong> {numberToWords(cn.totalAmount || 0)}
            </div>

            <div className="giv-declaration">
              <strong>Declaration:</strong> This credit note cancels the tax charged on the original supply and adjusts the return in GSTR-1 / GSTR-3B for the period in which it is issued. It is a system-generated document and does not require a physical signature.
            </div>
          </div>

          <div className="giv-footer">
            This is a computer-generated GST credit note | {cn.creditNoteNumber} | Generated on {new Date().toLocaleDateString()}
          </div>
        </div>
      </div>
    </>
